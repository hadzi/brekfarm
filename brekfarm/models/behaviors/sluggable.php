<?php
/**
 * Behavior which manages model slugs
 *
 * @todo implement support for scopes, see #breta01-13
 */
...
/**
 * Generate slug for model record
 *
 * @param string $string
 ...
 */
	private function __slug($string, $settings) {
		if ($settings['translation'] && is_array($settings['translation'])) {
			if (count($settings['translation']) >= 2 && count($settings['translation']) % 2 == 0) {
				for ($i = 0, $limit = count($settings['translation']); $i < $limit; $i += 2) {
			...
			} elseif (count($settings['translation']) == 1) {
				$string = strtr($string, $settings['translation'][0]);
			}
			$string = low($string);
		} elseif ($settings['translation'] && is_string($settings['translation']) && in_array(low($settings['translation']), array('utf-8', 'iso-8859-1'))) {
			$translations = array(
				'iso-8859-1' => array(
